def decipher_vigenere(key: str, text: str, lan="en"):
    ...
    for i in range(len(text)):
        index = len(language_options[lan]) - (language_options[lan].index(key[i]) - language_options[lan].index(text[i]))
        value += language_options[lan][index] if index < len(language_options[lan]) else language_options[lan][index-len(language_options[lan])]
